...
class AnimationController {
    ...
    private readonly viewZoom = 120
    private readonly numberOfStars = 800 // Reduced significantly for Lighthouse Performance score
    ...
    private setupTimeline() {
        this.timeline
            .to(this, {
                time: 1,
                duration: 10, // Faster animation speed
                repeat: -1,
                ease: "none",
    ...
    // 投影点
    public showProjectedDot(position: Vector3D, sizeFactor: number, color: string = 'white') {
        ...
        if (position.z > newCameraZ) {
            ...
            const sw = 400 * sizeFactor / dotDepthFromCamera // Adjusted size factor
            
            this.ctx.lineWidth = sw
            this.ctx.fillStyle = color
            
            this.ctx.beginPath()
            this.ctx.arc(x, y, 0.8, 0, Math.PI * 2) // Finer, sharper dots
            this.ctx.fill()
        }
    }
    ...
    private drawStartDot() {
        if (this.time > this.changeEventTime) {
            ...
            this.showProjectedDot(position, 2.5, 'white')
        }
    }
    ...
    public render() {
        ...
        ctx.fillStyle = 'black' // User requested black background
...
class Star {
    ...
    constructor(cameraZ: number, cameraTravelDistance: number) {
        ...
        // Golden and Silver combination
        const colors = [
            'rgba(251, 191, 36, 0.8)',   // Deep Gold
            'rgba(252, 211, 77, 0.6)',   // Light Gold
            'rgba(253, 230, 138, 0.9)',  // Pale Gold
            'rgba(209, 213, 219, 0.8)',  // Silver
            'rgba(229, 231, 235, 0.6)',  // Light Silver
            'rgba(255, 255, 255, 0.9)'   // Bright Silver/White
        ]
